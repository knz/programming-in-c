...
1. Implement the missing ``print_tree()`` function
   ...
   You can test your ``print_tree()`` implementation with ``./encode <
   /dev/null``. ``encode`` will use the fixed tree generated by
   ``fixed_tree()`` in this case.  Verify that the output matches that
   tree.
